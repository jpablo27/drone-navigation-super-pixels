		...
		)
{
	...
	for (int i = 0; i < sz; i++)
	{
		...
		if(conexiones[i][1]==0){
			...
			cfcLAB(i,i,ad_mat,conexiones,sz-i,group_i,kr,kg,kb,avg_r,avg_g,avg_b,g_list,t_flag);

			if(t_flag){

				for(int t_i=0; t_i< temprLAB.size(); t_i++)
					conexiones[temprLAB[t_i]][2]=group_tLAB;

				g_list[conexiones[temprLAB.back()][2]].insert(g_list[conexiones[temprLAB.back()][2]].end(), temprLAB.begin(),temprLAB.end());
				t_flag = false;

			}else{
				g_list.push_back(temprLAB);
				group_i++;
...
void cfcLAB(
	...
	for(int i=1; i<sz; i++){
		...
		if((ad_mat.at<double>(y,x+i)==1)&&(color_d<thLAB)){


			if(conexiones[x+i][1]==0){
				conexiones[x+i][1]=1;
				conexiones[x+i][2]=conexiones[y][2];
				temprLAB.push_back(x+i);
				...
				colcont++;
				cfcLAB(x+i,x+i,ad_mat,conexiones,(sz-i),group_i,kr,kg,kb,avg_r,avg_g,avg_b,g_list,t_flag);

			}else if(conexiones[y][2]!=conexiones[x+i][2]){

				if(!t_flag){
					group_tLAB=conexiones[x+i][2];
					t_flag=true;
